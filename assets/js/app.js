(function () {
  const app = {
    ...
    data: null,
    defaultLocation: '42.4396,-76.4970',
  ...
  app.getLocationNameFromLatLng = (latlng) => {
    const url = `https://maps.googleapis.com/maps/api/geocode/json?latlng=${latlng}&key=AIzaSyAyTa83D0I1xIL1MUeHISKCmznO9-5NXdU&result_type=locality`;
    let result = '';
    fetch(url)
      .then(response => {
        if (response.ok) {
  ...
  app.getWeather = (latlng) => {
    const url = `https://api.darksky.net/forecast/26d527885375a09d6af95508c1b281d2/${latlng}/?exclude=[minutely,hourly]`;
    app.showLoading();
    app.getLocationNameFromLatLng(`${latlng}`);
    fetch(url, {
        method: 'GET',
